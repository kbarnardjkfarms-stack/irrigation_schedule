...
function DeleteEntryButton({ onDelete, label = "this entry" }) {
  return (
    <button
      onClick={() => { if (window.confirm(`Delete ${label}? This can't be undone.`)) onDelete(); }}
      title="Delete"
      style={{
        background: "transparent", border: "none", color: "#5b6478", cursor: "pointer",
        padding: 4, display: "inline-flex", alignItems: "center", borderRadius: 4,
      }}
      onMouseEnter={(e) => { e.currentTarget.style.color = "#e08787"; }}
...
----------------------------------------------------------------*/
function BayDetail({ bay, data, stats, customers, varieties, readOnly, onAddTubeCheck, onAddCwtRun, onUpdateZoneCustomer, onUpdateZoneVariety, onDeleteTubeCheck, onDeleteCwtRun }) {
  ...
  const tubeChecks = (zoneData.tubeChecks || []).map((c, i) => ({ ...c, _idx: i })).reverse();
  const cwtRuns = (zoneData.cwtRuns || []).map((r, i) => ({ ...r, _idx: i })).reverse();
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
            <div style={{ background: "#141b28", border: "1px solid #232d40", borderRadius: 10, padding: 16 }}>
              ...
              <div style={{ marginTop: 14, maxHeight: 150, overflowY: "auto" }}>
                ...
                {tubeChecks.map((c) => (
                  <div key={c._idx} style={{ fontSize: 12, color: "#c7cede", padding: "6px 0", borderTop: "1px solid #232d40", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 6 }}>
                    <div>
                      <b>{c.date}</b> — {c.tubesRemaining} empty ({zone.tubeCount - c.tubesRemaining} filled)
                      {c.note && <div style={{ color: "#8790a3" }}>{c.note}</div>}
                    </div>
                    {!readOnly && <DeleteEntryButton label="this tube check" onDelete={() => onDeleteTubeCheck(bay.id, zone.id, c._idx)} />}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ background: "#141b28", border: "1px solid #232d40", borderRadius: 10, padding: 16 }}>
              ...
              <div style={{ marginTop: 14, maxHeight: 150, overflowY: "auto" }}>
                ...
                {cwtRuns.map((r) => (
                  <div key={r._idx} style={{ fontSize: 12, color: "#c7cede", padding: "6px 0", borderTop: "1px solid #232d40", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 6 }}>
                    <span><b>{r.date}</b> — {r.dest}</span>
                    <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ color: "#f2c14e" }}>{fmt(r.cwt)} cwt</span>
                      {!readOnly && <DeleteEntryButton label="this load" onDelete={() => onDeleteCwtRun(bay.id, zone.id, r._idx)} />}
                    </span>
                  </div>
                ))}
              </div>
...
function TemperatureTab({ bays, dataById, onAddTemp, onDeleteTemp, readOnly }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div>
        <div style={{ fontWeight: 700, marginBottom: 10, color: "#eef1f6" }}>{bay?.name} — recent readings</div>
        <div style={{ fontSize: 11.5, color: "#6f7890", marginBottom: 8 }}>Most recent first. Delete a bad or duplicate entry here.</div>
        <div style={{ overflowX: "auto", border: "1px solid #232d40", borderRadius: 10, maxHeight: 280, overflowY: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: "#141b28", textAlign: "left" }}>
            ...
            <tbody>
              {logs.length === 0 && <tr><td colSpan={5} style={{ ...tdStyle, color: "#5b6478" }}>No readings logged for this bay yet.</td></tr>}
              {logs.map((l, i) => i).reverse().map((originalIdx) => {
                const l = logs[originalIdx];
                return (
                  <tr key={originalIdx} style={{ borderTop: "1px solid #232d40" }}>
                    <td style={tdStyle}><b style={{ color: "#eef1f6" }}>{l.date}</b></td>
                    <td style={tdStyle}>{l.pipeNumber}</td>
                    <td style={{ ...tdStyle, color: l.position === "Top" ? "#f2c14e" : "#5fb0d6" }}>{l.position}</td>
                    <td style={tdStyle}>{l.temp}°F</td>
                    <td style={tdStyle}>{!readOnly && <DeleteEntryButton label="this reading" onDelete={() => onDeleteTemp(bayId, originalIdx)} />}</td>
                  </tr>
                );
              })}
            </tbody>
...
----------------------------------------------------------------*/
function SproutNipTab({ bays, dataById, customers, products, applicators, readOnly, onAddSproutApplication, onDeleteSproutApplication, onAddProduct, onUpdateProductRestrictions, onAddApplicator }) {
  const [bayId, setBayId] = useState(bays[0]?.id);
  const bay = bays.find((b) => b.id === bayId);
  const [zoneId, setZoneId] = useState(bay?.zones[0]?.id);
      ...
      <div>
        ...
        <div style={{ overflowX: "auto", border: "1px solid #232d40", borderRadius: 10 }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
              {history.length === 0 && <tr><td colSpan={6} style={{ ...tdStyle, color: "#5b6478" }}>No applications logged for this field yet.</td></tr>}
              {history.map((a) => (
                <tr key={a.id} style={{ borderTop: "1px solid #232d40" }}>
                  ...
                  <td style={tdStyle}>
                    {!readOnly && <DeleteEntryButton label="this application" onDelete={() => onDeleteSproutApplication(bayId, zoneId, a.id)} />}
                  </td>
                </tr>
              ))}
            </tbody>
...
export default function PotatoStorage() {
  ...
  const onDeleteTubeCheck = useCallback((bayId, zoneId, idx) => {
    updateZoneData(bayId, zoneId, (zd) => ({ ...zd, tubeChecks: (zd.tubeChecks || []).filter((_, i) => i !== idx) }));
  }, [updateZoneData]);
  ...
  const onDeleteCwtRun = useCallback((bayId, zoneId, idx) => {
    updateZoneData(bayId, zoneId, (zd) => ({ ...zd, cwtRuns: (zd.cwtRuns || []).filter((_, i) => i !== idx) }));
  }, [updateZoneData]);
  ...
  const onDeleteTemp = useCallback((bayId, idx) => {
    if (isReadOnly) return;
    setDataById((prev) => {
      const bayData = prev[bayId] || {};
      const nextBayData = { ...bayData, tempLogs: (bayData.tempLogs || []).filter((_, i) => i !== idx) };
      const next = { ...prev, [bayId]: nextBayData };
      saveJSON(bayDataKey(bayId), nextBayData);
      return next;
    });
            ...
            {locationBays.length === 0 ? (
              ...
              <>
                ...
                <BayDetail bay={selectedBay} data={displayDataById[selectedBay.id] || emptyBayData(selectedBay)} stats={statsById[selectedBay.id]}
                  ...
                  onDeleteTubeCheck={onDeleteTubeCheck} onDeleteCwtRun={onDeleteCwtRun} />
              </>
            )}
          </div>
        )}
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
            {locationBays.length === 0 ? <EmptySiteNotice onManage={() => setTab("manage")} /> : (
              <SproutNipTab bays={locationBays} dataById={displayDataById} customers={customers} products={products} applicators={applicators}
                ...
                onAddProduct={onAddProduct} onUpdateProductRestrictions={onUpdateProductRestrictions} onAddApplicator={onAddApplicator} />
            )}
          </div>
        )}
      </div>
